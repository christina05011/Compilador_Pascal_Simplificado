...
struct parser_class {
    ...
    void Program() { //Program → PROGRAM id /'(id)'|e/; ConstBlock VarBlock TypeBlock FunctionBlock ProcedureBlock MainCode
        ...
        if (token_tag[0] != "PROGRAM") add_error("ErrorProgram: Find '" + token_tag[1] + "' but it must be 'program'.", follows, 6);
            ...
            if (token_tag[0] != "ID") add_error("ErrorProgram: Find '" + token_tag[1] + "' but it must be 'id'.", follows, 6);
            else { add_node(tree.root, "id"); next_token(); }
            if (token_tag[0] == "DEL_OP") {
                add_node(tree.root, "("); next_token();
                if (token_tag[0] != "ID") add_error("ErrorProgram: Find '" + token_tag[1] + "' but it must be 'id'.", follows, 6);
                else { add_node(tree.root, "id"); next_token(); }
                if (token_tag[0] != "DEL_CP") add_error("ErrorProgram: Find '" + token_tag[1] + "' but it must be ')'.", follows, 6);
                else { add_node(tree.root, ")"); next_token(); }
            }
            if (token_tag[0] != "DEL_SEMC") add_error("ErrorProgram: Find '" + token_tag[1] + "' but it must be ';'.", follows, 6);
            else { add_node(tree.root, ";"); next_token(); }
        }
